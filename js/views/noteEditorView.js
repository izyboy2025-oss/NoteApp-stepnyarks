function renderNoteEditorView(root, { note, onBack, onDelete, onTogglePin }) {
  root.innerHTML = `
    <div class="view-col">
      ...
      <div class="sync-footer" id="sync-footer">Backed up</div>
    </div>
  `
  ...
  const footer = root.querySelector('#sync-footer')
  ...
  function updateFooter() {
    if (Notes.syncStatus === 'saving') footer.textContent = 'Saving…'
    else if (Notes.syncStatus === 'error') footer.textContent = 'Backup failed — will retry'
    else footer.textContent = 'Backed up'
  }
  updateFooter()
  const unsubscribe = Notes.onChange(updateFooter)
